...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("work:created", (work: Task) => {
      setTasks((prev) => [work, ...prev]);
    });

    socket.on("work:updated", (work: Task) => {
      setTasks((prev) =>
        prev.map((item) => (item._id === work._id ? work : item)),
      );
    });
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-3xl border border-dashed border-slate-200 p-20 text-center">
              ...
              <p className="text-slate-400 text-sm max-w-xs mx-auto">
                No tasks have been assigned to you yet. Sit tight or check back
                later!
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <h3
                          className={`text-lg font-bold transition-all ${task.status === TaskStatus.COMPLETED ? "text-slate-400 line-through" : "text-slate-900"}`}
                        >
                          {task.title}
                        </h3>
                        <div className="flex items-center gap-2 shrink-0">
                          <span
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${getStatusStyles(task.status)}`}
                          >
                            {task.status.replace("_", " ")}
                          </span>
                          <span
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${getPriorityColor(task.priority)}`}
                          >
                            {task.priority}
                          </span>
                        </div>
                      ...
                      <p
                        className={`text-sm mb-4 leading-relaxed ${task.status === TaskStatus.COMPLETED ? "text-slate-300" : "text-slate-500"}`}
                      >
                        {task.description ||
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center gap-1.5 text-xs font-bold text-slate-400">
                            ...
                            <span>
                              Due{" "}
                              {new Date(task.overDue).toLocaleDateString(
                                undefined,
                                {
                                  month: "short",
                                  day: "numeric",
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] font-black text-indigo-600 px-2 py-0.5 bg-indigo-50 rounded-md">
                              ...
                              {task.assignedBy?.workRole || "Admin"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="relative group/select">
                          ...
                          <ChevronRight
                            className={`absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none rotate-90 ${task.status === TaskStatus.TO_DO ? "text-white" : ""}`}
                          />
                        </div>
                      </div>
                    </div>
